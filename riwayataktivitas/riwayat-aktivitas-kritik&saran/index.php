...
<body id="page-top">
            ...
            <li class="nav-item active">
                ...
                <div id="collapsePages4" class="collapse" aria-labelledby="headingPages3"
                    ...
                    <div class="bg-white py-2 collapse-inner rounded">
                        ...
                        <a class="collapse-item active" href="/gate/admin/riwayataktivitas/riwayat-aktivitas-kritik&amp;saran/"><b>Kritik & Saran</b></a>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <section id="riwayat" class="container-fluid">
                <style>
                    ...
                    <!-- Page Heading -->
                    <h1 class="h4 mb-2 text-gray-800">Aktivitas Kritik & Saran</h1>
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <!-- Script Fitur Pencarian -->
                            <form action="<?php echo $_SERVER["PHP_SELF"];?>" method="get" class="d-flex">
                                <input type="text" name="cari" class="form-control me-2" aria-label="Search" placeholder="Cari nama depan, nama belakang atau jenis aktivitas">
                            ...
                            <div class="table-responsive">
                                ...
                                <tbody>
                                    ...
                                    <?php
				                        ...
				                        $batas4 = 10;
				                        $halaman4 = isset($_GET['halaman4'])?(int)$_GET['halaman4'] : 1;
				                        $halaman_awal4 = ($halaman4>1) ? ($halaman4 * $batas4) - $batas4 : 0;
			                        	$Sebelum4 = $halaman4 - 1;
			                        	$lanjut4 = $halaman4 + 1;
				                        $id_log = 1;
				                        $data4 = mysqli_query($koneksi,"select * from log_kritik");
				                        $jumlah_data4 = mysqli_num_rows($data4);
				                        $total_halaman4 = ceil($jumlah_data4 / $batas4);
				                        $sql4 = mysqli_query($koneksi,"select * from log_kritik limit $halaman_awal4, $batas4");
				                        $id_log = $halaman_awal4+1;
				                        
				                        if(isset($_GET['cari'])){
		                                $cari = $_GET['cari'];
		                                $sql4 = mysqli_query($koneksi,"SELECT * FROM log_kritik WHERE namdep like '%".$cari."%' OR namleng like '%".$cari."%' OR keterangan_log like '%".$cari."%' ORDER BY id_log ASC");
	                                    }else{
		                                $data4 = mysqli_query($koneksi,"SELECT * FROM log_kritik ORDER BY id_log ASC");
	                                    }
	                                    
	                                    if(mysqli_num_rows($sql4)){
			                        	while($resl4 = mysqli_fetch_array($sql4)){
				                	?>
                                        <tr>
                                            <th><center><?php echo htmlspecialchars($id_log++); ?></center></th>
                                            <th><center><?php echo htmlspecialchars($resl4[ 'namdep']); ?></center></th>
                                            <th><center><?php echo htmlspecialchars($resl4[ 'namleng']); ?></center></th>
                                            <th><center><?php echo date('d F Y | H:i A',strtotime($resl4['tgl'])); ?></center></th>
                                            <th><center><?php echo htmlspecialchars($resl4[ 'keterangan_log']); ?></center></th>
                                        </tr>
                                        <?php
                                            }}else{
                                                echo '<tr><th colspan="5"><center><font color="FF0000">Tidak ada data, silahkan masukkan nama depan, nama belakang & jenis aktivitas kembali &#128522;</font></center></th></tr>';
                                            }?>
                                    </tbody>
                                </table>
                                ...
                                <!-- Script menampilkan total data -->               
                                    <?php 
                                        $tampil4 = "SELECT * FROM log_kritik";
                                        $hasil4 = mysqli_query($koneksi,$tampil4);
                                        $jmldata4= mysqli_num_rows($hasil4);
                                        $jmlhalaman4 = ceil($jmldata4/$batas4);
                                        echo "<b><font color='#c80000'>Jumlah Data : $jmldata4</font></b>";
                                    ?>
                                ...
                                <nav>
			                        <ul class="pagination justify-content-center">
				                        <li class="page-item">
					                    <a class="page-link" <?php if($halaman4 > 1){ echo "href='?halaman4=$Sebelum4'"; } ?>><b>&laquo;</b></a>
				                        </li>
				                    <?php 
				                        for($x=1;$x<=$total_halaman4;$x++){
					                ?> 
					                    <li class="page-item">
					                    <a class="page-link" href="?halaman4=<?php echo htmlspecialchars($x) ?>"><?php echo htmlspecialchars($x); ?></a>
					                    </li>
					                <?php
				                    }
				                    ?>				
				                        <li class="page-item">
					                    <a  class="page-link" <?php if($halaman4 < $total_halaman4) { echo "href='?halaman4=$lanjut4'"; } ?>><b>&raquo;</b></a>
				                        </li>
			                        </ul>
		                        </nav>
    ...
    <script>
	  history.pushState({}, "", '/gate/admin/riwayataktivitas/riwayat-aktivitas-kritik&saran/');
	</script>
