<?php
    ...
    // Untuk penhubung agar tidak bisa memasuki halaman lain //
    if(!isset($_SESSION['email'])){
        header("location:/gate/lupakatasandi/?pesan=masukkan-email");
    }
        ...
        if ($emailCheckResult) {
            ...
            if (mysqli_num_rows($emailCheckResult) > 0) {
                ...
                if ($updateResult) {
                    ...
                    $message = "Silahkan melakukan verifikasi dengan kode tertera sebagai berikut : $otppass";
                    $sender = 'From: ppkhmft22@kthngudiwaluyo.com';

                    if (mail($email, $subject, $message, $sender)) {
                        
                        $message = "<b><center>Kami telah mengirimkan kode verifikasi ke email Anda <font color='0d6efd'>$email</font></center></b>";
...
if(isset($_POST['changePassword'])){
    ...
    if (strlen($_POST['password']) < 8) {
        ...
        if ($_POST['password'] != $_POST['confirmPassword']) {
            $errors['password_error'] = "<b><center><font color='FF0000'>Kata sandi tidak cocok</font></center></b>";
        } else {
            ...
            session_unset($email);
            session_destroy();
            header('location: /gate/?pesan=berhasilgantikatasandi');
        }
    }
}